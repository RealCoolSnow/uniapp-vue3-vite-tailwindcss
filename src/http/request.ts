...
class Request implements RequestInterface {
  ...
  getUrl(config: RequestConfig) {
    ...
    if (!abs) {
      const f = this.config.slashAbsoluteUrl
      if (f) {
        abs = /^\/([\w.]+\/?)\S*/.test(url)
      }
    }
  ...
  getContentType(config: RequestConfig) {
    const type = config.contentType || 'json'
    const charset = this.config.encoding || 'UTF-8'
    if (type === 'json') {
  ...
  _success(
    ...
    if (res.statusCode >= 200 && res.statusCode <= 302) {
      ...
      const skip = _config.skipInterceptorResponse
      // 走全局的拦截器，
      if (
